...
<div class="content-wrapper">
	...
	<section class="content">
		<div class="row">
			<div class="col-md-12">
				<p><a href="{{ route('registrar.subjects') }}" class="btn btn-danger"><i class="fa fa-arrow-left"></i> Back to Subjects</a></p>
				...
				<div class="box box-danger">
					...
					<div class="box-body">
						...
						<form action="{{ route('registrar.update.subject.post') }}" method="POST" autocomplete="off">
							{{ csrf_field() }}
										...
										<select class="form-control" id="prerequisite" name="prerequisite">
											...
											@if(count($subjects) > 0)
												@foreach($subjects as $s)
													@if($s->id != $subject->id)
														<option value="{{ $s->id }}" {{ $s->id == $subject->id ? 'selected' : '' }}>{{ $s->code }}</option>
													@endif
												@endforeach
											@else
							...
							<div class="form-group">
								<button type="submit" class="btn btn-danger"><i class="fa fa-floppy-o"></i> Update Subject</button>
							</div>
						</form>
					</div>
